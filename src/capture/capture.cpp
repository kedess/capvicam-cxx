...
capvicam::Capture::Capture(const char * path, uint16_t width, uint16_t height) {
    ...
    if (-1 == this->fd) {
        throw std::runtime_error("Could not open device (" + std::string(path) + "). " + std::string(strerror(errno)));
    }
    std::cout << "[INFO ]: Open device (" << path << "). successfully" << std::endl;
    struct v4l2_capability * caps = new struct v4l2_capability;
    if (-1 == ioctl(fd, VIDIOC_QUERYCAP, caps)) {
        throw std::runtime_error("Could not read device capability. " + std::string(strerror(errno)));
    }
    std::cout << "[INFO ]: Device capability driver = " << caps->driver
                                        ...
                                        << "." << ((caps->version >> 8) & 0xFF) 
                                        << "." << (caps->version & 0xFF) <<  std::endl;
    if (!(caps->device_caps & V4L2_CAP_STREAMING) && (caps->device_caps & V4L2_CAP_VIDEO_CAPTURE)) {
    ...
    if (-1 == ioctl (fd, VIDIOC_S_FMT, &fmt)) {
        throw std::runtime_error("Could not set format params. " + std::string(strerror(errno)));
    }
    this->create_buffers();
}
...
capvicam::Capture::~Capture(){
    ...
    if (ioctl (fd, VIDIOC_STREAMOFF, &kind) == -1) {
        std::cout << "[ERROR]: an error occurred during the stop streaming. " << strerror(errno) << std::endl;
        return;
    }
...
void capvicam::Capture::run(OnImageReceived callback) {
    ...
    while (-1 == signal_num) {
        ...
        if(ff == -1) {
            throw std::runtime_error("An error occurred during streaming. " + std::string(strerror(errno)));
        }
